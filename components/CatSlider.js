...
export default function CategoriesCarousel() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await fetch("https://frischly-server.onrender.com/api/categories?limit=1000");
...
for (let i = 0; i < limitedCategories.length; i += 2) {
  ...
  const renderCategory = (category) => (
    ...
    >
      <View style={styles.imageWrapper}>
        <Image
          source={{ uri: category.image.replace("/upload/", "/upload/q_1/") }}
          style={styles.image}
          resizeMode="cover"
        /> 
      </View>
  ...
  return (
    <View style={{ height: ITEM_HEIGHT, marginTop: 30, backgroundColor: '#fff' }}>
      <View style={styles.header}>
        <Text style={styles.headerText}>Shop by Category</Text>
        <View style={styles.headerRight}>
          <TouchableOpacity style={styles.allButton} onPress={() => router.push("/shop")}>
...
const styles = StyleSheet.create({
  card: { width: ITEM_WIDTH, margin: 5, backgroundColor: "transparent", padding: 8 },

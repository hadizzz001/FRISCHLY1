...
const ProductPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`https://frischly-server.onrender.com/api/products/${search}`);
    ...
    <View style={{ flex: 1, backgroundColor: "#fff" }}>
      ...
      <ScrollView style={styles.container}>
        ...
        >
          ...
          >
            <TouchableOpacity activeOpacity={1} onPress={(e) => e.stopPropagation()} style={styles.zoomContainer}>
              <Image
        ...
        {/* Image Swiper */}
        <Swiper style={styles.swiper} showsPagination loop dotStyle={styles.dot} activeDotStyle={styles.activeDot}>
          {[picture].map((src, i) => (
            <View key={i} style={styles.slide}>
              <TouchableOpacity onPress={() => setZoomedImg(src.replace("/upload/", "/upload/q_20/"))}>
                <Image source={{ uri: src.replace('/upload/', '/upload/q_20/') }} style={styles.swiperImage} resizeMode="contain" />
              </TouchableOpacity>
            </View>
          ))}
        </Swiper>
        ...
        <View style={styles.info}>
          ...
          {subcategory?.name && <Text>{parentCategory?.name} / {subcategory?.name}</Text>}
          ...
          {isInCart ? (
            ...
            <>
              <QuantitySelector initialQty={quantity} onChange={setQuantity} productId={_id} />
              {!isOutOfStock ? (
      ...
      <View style={styles.tabBar}>
        ...
        <TouchableOpacity style={styles.tabButton} onPress={() => setProfileOpen(true)}>
      ...
      {profileOpen && (
        <View style={styles.overlay}>
          ...
          <ScrollView contentContainerStyle={styles.overlayContentProfile}>
            ...
            <TouchableOpacity style={styles.row} onPress={async () => { await AsyncStorage.removeItem("userData"); await AsyncStorage.setItem("guest", "false"); setProfileOpen(false); router.replace("/start"); }}>
        ...
        <View style={styles.overlay}>
          ...
          <ScrollView contentContainerStyle={styles.overlayContentMenu}>
            ...
            {categories.slice(0, 8).map(cat => (
              <TouchableOpacity key={cat._id} onPress={() => { setMenuOpen(false); router.push(`/shop?category=${cat._id}`); }}>
                <Text style={styles.item}>{cat.name}</Text>
              </TouchableOpacity>
            ))}
            <TouchableOpacity onPress={() => { setMenuOpen(false); router.push("/shop"); }}>
...
const styles = StyleSheet.create({
  ...
  button: { backgroundColor: '#000', padding: 12, borderRadius: 6, marginTop: 8 },
  buttonText: { color: '#fff', textAlign: 'center', fontWeight: 'bold' },
  dot: { backgroundColor: 'rgba(0,0,0,0.2)', width: 8, height: 8, borderRadius: 4, margin: 3 },
  activeDot: { backgroundColor: '#000', width: 10, height: 10, borderRadius: 5, margin: 3 },
  center: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  overlay: { position: "absolute", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(255,255,255 )", zIndex: 100, paddingTop: 50 },
  imageWrapper: { maxWidth: '90%', maxHeight: '90%' },
  zoomImage: { width: width * 0.9, height: width * 0.9, borderRadius: 10, resizeMode: 'contain' },
  backButton: { marginRight: 6, padding: 4, backgroundColor: '#fff' },

  // Tab Bar
  tabBar: { flexDirection: "row", justifyContent: "space-around", paddingVertical: 12, backgroundColor: "#fff" },
  tabButton: { alignItems: "center", justifyContent: "center" },
  cartBadge: { position: "absolute", right: -6, top: -3, backgroundColor: "red", borderRadius: 8, width: 12, height: 12 },
  closeBtn: { position: "absolute", top: 40, right: 20, width: 40, height: 40, borderRadius: 20, justifyContent: "center", alignItems: "center", zIndex: 200 },

  // Overlay contents
  overlayContentProfile: { paddingTop: 100, paddingHorizontal: 20, alignItems: "flex-start" },
  overlayContentMenu: { flexGrow: 1, justifyContent: "center", alignItems: "center", paddingHorizontal: 20 },
  item: { fontSize: 16, marginVertical: 10, color: "#000" },
  row: { flexDirection: "row", alignItems: "center", marginVertical: 8 },
  finalPrice: { fontSize: 18, fontWeight: "bold", marginTop: 8, marginBottom: 8 },
